    ...
    <form className="p-7 bg-[#43035f] overflow-y-auto min-h-[200px] shadow-[8px_8px_15px_5px_rgba(0,0,0,0.6)]" method="post" onSubmit={handleSubmit}>
      {ingredients.map((ing, index) => (
        <div className="flex flex-row items-center gap-3 mb-6" key={index}>
          <input className="bg-white xl:w-[400px] 2xl:w-[500px] p-3 rounded-lg focus:outline-2 focus:outline-[#41c398]" onChange={(e) => handleChange(e.target.value, index)} value={ing} type="text" placeholder={`Ingrediente ${index + 1}`} />
